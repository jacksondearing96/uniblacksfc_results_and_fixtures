...
function TestProcessLocation(test) {
  ...
  game.location = 'University Oval';
  ExpectEqual(ProcessLocation(game), '', test, 'Process location: BN#1');

  game.location = "Fred Bloch Oval";
  ExpectEqual(ProcessLocation(game), '', test, 'Process location: Chocka');

  game.location = "Actual Oval Name";
  ExpectEqual(ProcessLocation(game), '(Actual Oval Name)', test, 'Process location: away oval');
}

function TestExpandDate(test) {
  ExpectEqual(ExpandDate('Mon 7 Jul'), 'Monday 7 July', test, 'Expand date: basic');
  ExpectEqual(ExpandDate('Mon, Tue, Wed, Thu, Fri, Sat, Sun, Jan Feb Mar, Apr, May, Jun, Jul, Aug, Sep, Nov, Dec'),
