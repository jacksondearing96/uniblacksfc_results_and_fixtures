...
function FutureContentArrayToJson(row_content) {
  return {
    ...
    date: row_content[4],
    opposition: row_content[5],
    location: row_content[6],
    location_nickname: row_content[7],
    time: row_content[8],
...
override_nicknames = { "Angle Vale": "Angle Fail", "St Paul's OS": "Saint Paul" };
override_ground_names = {};
...
function FormatPastGamesHelper(past_teams) {
  ...
  for (let i in past_teams) {
    LoadHTMLTemplateToList('/past-game.html', past_games_HTML, past_teams[i], '#past-games-container');
  }
}

function FormatFutureGamesHelper(future_teams) {
  ...
  for (let i in future_teams) {
    ProcessLocation(future_teams[i]); // Add the braces around the real location and ignore home ground real names.
    LoadHTMLTemplateToList('/future-game.html', future_games_HTML, future_teams[i], '#future-games-container');
  }
}

function UpdateTablesHelper(past_teams, future_teams, callback) {

  InitialiseWinningVerbs();

  const opposition_col_index = 5;
  const result_col_index = 6;
  ...
  const location_nickname_col_index = 6;
